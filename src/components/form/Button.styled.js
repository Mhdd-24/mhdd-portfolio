...
import { blue, typeScale, black, green, red } from "../../utils";
...
export const Button = styled.button`
  ...
  &:hover {
    background-color: ${(props) => (props.bg ? props.bg["70"] : red["70"])};
  ...
  &:active,
  &:focus {
    background-color: ${(props) => (props.bg ? props.bg["80"] : green["80"])};
    box-shadow: 4px 4px 0 ${(props) => (props.bg ? props.bg["00"] : green["00"])};
  }

  &:disabled {
    cursor: not-allowed;
    opacity: 0.5;
    box-shadow: 2px 2px 0 ${(props) => (props.bg ? props.bg["00"] : black["00"])};
  }
`;
